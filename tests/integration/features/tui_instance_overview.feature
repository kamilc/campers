...
Scenario: Widget displays correct counts
  ...
  When I view the TUI
  Then overview widget shows "Running: 2  Stopped: 3  N/A"

@smoke @localstack
Scenario: Widget handles empty instance list
  Given I have 0 instances
  When I view the TUI
  Then overview widget shows "Running: 0  Stopped: 0  N/A"

@smoke @localstack
Scenario: Widget handles EC2 API errors gracefully
  Given TUI is displaying "Running: 2  Stopped: 1  N/A"
  When EC2 API fails
  And 30 seconds pass
  Then overview widget shows "Running: 2  Stopped: 1  N/A"
...
Scenario: Widget queries all regions
  ...
  When I view the TUI
  Then overview widget shows "Running: 1  Stopped: 2  N/A"

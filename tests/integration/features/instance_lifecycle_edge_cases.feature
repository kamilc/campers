...
Scenario: Terminate waiter timeout raises error
  ...
  When I run "moondock stop moondock-myproject-main" with timeout override to 1 second
...
Scenario: Volume size retrieval handles missing volume gracefully
  Given an instance with name "moondock-myproject-main"
...
Scenario: Instance operations fail with AWS permission errors
  ...
  And an instance with name "moondock-myproject-main" exists
  When I run "moondock stop moondock-myproject-main"
...
Scenario: Stop instance by ID when multiple instances with same name
  ...
  When I run "moondock stop i-12345"
